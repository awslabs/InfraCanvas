...
def get_bedrock_response_stream(file, qa_string, context_history, iac_format, additional_notes):
    prompt = get_final_prompt(file, additional_notes, iac_format, qa_string, context_history)
...
def main():
    ...
    with st.sidebar:
        ...
        text_input = st.text_area("Enter additional details regarding architecture here")
    ...
    if st.session_state.questions:
        with st.sidebar.expander("Answer Questions"):
            submit, responses, iac_format, additional_notes = create_response_form(st.session_state.questions)
        ...
        if submit:
            ...
            with st.spinner(f"Generating {iac_format} code..."):
                ...
                if response_stream:
                    generated_code = stream_code_display(
                        iac_format,
                        response_stream
                    )
